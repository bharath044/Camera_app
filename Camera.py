...
def check_camera(camera_name, rtsp_url):
    ...
    for attempt in range(RETRIES):
        logging.info(f"[{camera_name}] Attempt {attempt+1}/{RETRIES}")
        try:
            reader = imageio.get_reader(rtsp_url, "ffmpeg")
            start_time = time.time()
            while time.time() - start_time < READ_FRAME_TIMEOUT_SEC:
                try:
            ...
            logging.warning(f"[{camera_name}] Could not open stream: {e}")
            time.sleep(1)

    # All retries failed → offline
    new_status = "offline"
    if prev_status != new_status:
...
if __name__ == "__main__":
    logging.info("📷 Multi-Camera Health Monitor Started")
    try:
        while True:
